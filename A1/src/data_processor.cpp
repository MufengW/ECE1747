...
void populate_linesArray(const std::string& data_buffer) {
    ...
    while (std::getline(ss, line) && line_count < g_particle_limit) {
        ...
        g_linesArray.push_back(line);
        ++line_count;
    }
    M_assert(g_linesArray.size() > 1, "There is only 1 particle!");
}
...
void set_chunk_boundaries() {
    ...
    size_t avg_lines_per_chunk = g_linesArray.size() / g_thread_count;
    size_t extra_lines = g_linesArray.size() % g_thread_count;
